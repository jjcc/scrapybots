...
class GeneralbotDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
...
class GeneralbotSpiderMiddlewarePostCrypto(object):
    """Post process for crypto scrapping
    """
    ...
    def process_spider_output(self, response, result, spider):
        ...
        # it has processed the response.
        url = response.request.url
        spider.logger.info(">>>process output in croypto mw")
        #spider.logger.info(url)
        # Must return an iterable of Request, dict or Item objects.
        for r in result:
            #spider.logger.info(i)
            yield r
            if 'coinmarket' not in response.url:
                continue
            #if type(r) == http.request.Request:
            #    spider.Request(r, callback=spider.parse)
            #print r
            pass
        #spider.logger.info("<<result:")
        #spider.logger.info(result)
...
#            yield r
    ...
    def spider_opened(self, spider):
        spider.logger.info('Spider opened(crypto: %s' % spider.name)
